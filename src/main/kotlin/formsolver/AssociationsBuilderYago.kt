...
internal class AssociationsBuilderYago(
    ...
    private fun <T> runQuery(queryStr: String, processResult: (ResultSet?) -> T): T {
        try {
            ...
            QueryExecutionFactory.sparqlService(FormSolver.yagoEndpoint, query).use { qExec ->
    ...
    private fun classAssociations(namespace2: String, tag: String, threshold: Int): String {
        ...
        while (x < synonyms.size) {
            ...
            val queryString = CommonData.yagoPrefix + "SELECT DISTINCT (?concretename AS ?" + queryTag + ") WHERE { ?class rdfs:label \"" + queryTag + "\"@eng . ?concrete rdf:type  ?class .?concrete rdfs:label ?concretename . FILTER (lang(?concretename) = 'eng') } LIMIT 1"

            val quantity = runQuery(queryString) { result ->
                if (result?.hasNext() == true) {
                    val quantity = this.getQuantity(namespace2, queryTag, false)
                    val sqs = result.next()
                    val obj = sqs.get(queryTag) // node2")
                    this.node = obj
    ...
    private fun getQuantity(namespace2: String, tag: String, predicate: Boolean): Int {
        ...
        val triple = "?class rdfs:label \"" + tag + "\"@eng . ?concrete rdf:type  ?class .?concrete rdfs:label ?concretename . FILTER (lang(?concretename) = 'eng')"
        ...
        val queryString = CommonData.yagoPrefix + "SELECT (COUNT(*) as ?count) " + "WHERE {" + triple + "}"
