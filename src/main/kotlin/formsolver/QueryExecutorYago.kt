...
class QueryExecutorYago internal constructor(
    ...
    internal fun runFinalQuery(maxEntries: Int) {
        ...
        for (g in forest) {
            ...
            while (ite.hasNext()) {
                val tri = ite.next()
                queryString = CommonData.yagoPrefix + "SELECT DISTINCT (?concretename AS ?" + tri.subject.toString() + ") WHERE { ?class rdfs:label \"" + tri.subject.toString() + "\"@eng . ?concrete rdf:type  ?class .?concrete rdfs:label ?concretename . FILTER (lang(?concretename) = 'eng') } LIMIT $maxEntries"
            ...
            val qExec = QueryExecutionFactory.sparqlService(FormSolver.yagoEndpoint, query2)
